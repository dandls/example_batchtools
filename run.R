...
run <- function(
  ...
  W.hat <- if (propensities) NULL else .5
  trt <- d$trt
  ...
  if ((causal_forest && !prognostic_effect) | propensities | marginal_mean) {
    ...
    if (attributes(d)$truth$mod == "normal") {
      cf <- causal_forest(X = as.matrix(d[, grep("^X", colnames(d))]),
        Y = d$y, W = (0:1)[d$trt], W.hat = NULL, # W.hat = W.hat if necessary!
    ...
    cf <- multi_arm_causal_forest(X = as.matrix(d[, grep("^X", colnames(d))]),
      Y = d$y, W = d$trt, W.hat = NULL, split.on.intercept = TRUE,
      stabilize.splits = stabilize.splits,
      min.node.size = min_size_group, sample.fraction = prt$fraction,
      mtry = mtry,
      num.trees = NumTrees, honesty = honesty)
...
# fun.cf2 <- function(instance, ...) {
#   run(instance, causal_forest = TRUE, propensities = FALSE,
#     prognostic_effect = FALSE, honesty = FALSE, stabilize.splits = FALSE)
# }
#
# fun.cf2honest <- function(instance, ...) {
#   run(instance, causal_forest = TRUE, propensities = FALSE,
#     prognostic_effect = FALSE, honesty = TRUE, stabilize.splits = FALSE)
# }
...
fun.mob <- function(instance, RMST = FALSE, ...) {
  ...
  # if ("rmst" %in% names(attributes(mob))) {
  #     mobrmst <- attr(mob, "rmst")
  #     return(mob, mobrmst)
  # }
  return(mob)
}
...
fun.bmhybridcox <- function(instance, ...) {
  ...
  if (FALSE) {
    ...
    test <- function(distr, setup.id = NULL, nsim = 800, methods = c("mob", "cf"),
      test.tf = FALSE, cox = FALSE, honesty = TRUE, propensities = FALSE, marginal_mean = FALSE, ...) {
      tmp <- which(lapply(setups, function(setup) setup$model) == distr)
      if (is.numeric(setup.id)) {
        tmp <- tmp[setup.id]
      }
      res <- lapply(setups[tmp], function(stp){
        d <- simulate(stp, nsim = nsim, dim = 10)
        if ("mob" %in% methods) {
          mob <- run(d, propensities = propensities,  Cox = cox,
            honesty = honesty, marginal_mean = marginal_mean, ...)
        } else {
          mob <- NA
        }
        if ("cf" %in% methods) {
          cf <- run(d, causal_forest = TRUE, honesty = honesty)
        } else {
          cf = NA
        }
        return(c(mob, cf))
      ...
      names(df) <- c("model4you", "causalforest")
      return(do.call("rbind", res))
    }

    ### normal
    set.seed(1234L)
    nn <- test(distr = "normal",  setup.id = 3L, methods = "cf", honesty = FALSE)
    set.seed(1234L)
    nn2 <- test(distr = "normal", setup.id = 3L, methods = "mob", honesty = FALSE,
      marginal_mean = TRUE, prognostic_effect = FALSE, propensities = TRUE)
    set.seed(1234L)
    nn3 <- test(distr = "normal", setup.id = 3L, methods = "mob", honesty = FALSE)
}
